...
4. src/main/resources/（classpath:/）（优先级最低）。
...
> Spring Boot中可供修改的配置，可参考[官方文档](https://docs.spring.io/spring-boot/docs/current/reference/html/appendix-application-properties.html#common-application-properties)或本教程**附录2 Spring Boot配置大全**。
...
application.properties文件内容如下：（启用dev profile，这个时候，服务器端口为`8080`）
