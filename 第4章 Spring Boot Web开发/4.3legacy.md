...
在Spring Boot中使用遗留Sevlet，可以通过`ServletRegistrationBean`注册。
...
当然，也可以使用**Servlet 3.0+**规范的注解方式来开发Servlet，然后配置组件扫描，将其注册到Servlet容器（例如，Tomcat）中。
...
在Spring Boot中，和Servlet类似，可以用`ServletListenerRegistrationBean`将其注册到web容器中。
...
在Spring Boot中，用`FilterRegistrationBean`将其注册到web容器中。
...
JSP 技术是以 Java 语言作为脚本语言的，JSP 网页为整个服务器端的 Java 库单元提供了一个接口来服务于HTTP的应用程序。JSP文件后缀名为 ***.jsp** 。
...
接下来，创建`/src/main/webapp/WEB-INF/jsp`目录，并在其下创建一个JSP文件hello.jsp。
...
在`/src/main/resources/static/images`下放置图片文件RoyElephant.png，供JSP文件访问静态资源演示用。
...
创建一个Spring Boot项目，选中**Spring MVC**依赖。

![image-20191123175352010](images/image-20191123175352010.png)

由于需要使用JSP，就要在pom文件中增加`tomcat-embed-jasper`这个JSP编译器。
...
在配置文件`application.properties`中，为JSP视图解析器设置前缀后缀属性。
...
创建JSP文件hello.jsp和控制器HelloController。在`/src/main/resources/static/images`下放置图片文件RoyElephant.png。
...
访问 [localhost:8080/helloServlet](http://localhost:8080/helloServlet) ，检查Servlet输出结果。
