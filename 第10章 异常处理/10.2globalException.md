...
在SpringBoot 统一异常处理的做法主要有两种：一是基于注解**ExceptionHandler**，二是实现接口**ErrorController**或继承扩展**AbstractErrorController**类（BasicErrorController就继承自这个类）。
