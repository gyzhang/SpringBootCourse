...
为了理解Spring Boot在Spring MVC应用中提供的全局异常处理机制，建议读者以调试方式启动Spring Boot应用，访问一个不存在的地址，跟踪代码，加深理解。

第一个断点，设置在`DispatcherServlet.doDispatch`方法。

第二个断点，设置在`org.apache.catalina.core.StandardHostValve.status`方法
...
private void status(Request request, Response response) {
    ...
    if (errorPage == null) {
        ...
        errorPage = context.findErrorPage(0);//ErrorPage[errorCode=0, location=/error]
    }
